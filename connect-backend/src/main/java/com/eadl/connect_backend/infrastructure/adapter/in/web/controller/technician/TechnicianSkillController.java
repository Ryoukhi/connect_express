...
public class TechnicianSkillController {
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "201", description = "Compétence créée", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TechnicianSkillDto.class))),
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Liste des compétences récupérée", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TechnicianSkillDto.class))),
                        @ApiResponse(responseCode = "403", description = "Accès interdit")
        })
        @PreAuthorize("hasRole('TECHNICIAN')")
        @GetMapping("/me")
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Liste des compétences filtrée par catégorie", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TechnicianSkillDto.class))),
                        @ApiResponse(responseCode = "403", description = "Accès interdit")
        })
        ...
        @Operation(summary = "Récupérer les compétences d'un technicien (Admin)")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Liste des compétences du technicien", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TechnicianSkillDto.class))),
                        @ApiResponse(responseCode = "403", description = "Accès interdit")
        })
        @PreAuthorize("hasRole('ADMIN')")
        @GetMapping("/technician/{technicianId}")
        public ResponseEntity<List<TechnicianSkillDto>> getSkillsByTechnicianId(
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Compétence mise à jour", content = @Content(mediaType = "application/json", schema = @Schema(implementation = TechnicianSkillDto.class))),
                        @ApiResponse(responseCode = "403", description = "Accès interdit")
        })
        @PreAuthorize("hasRole('TECHNICIAN')")
        @PutMapping("/{idSkill}")
        public ResponseEntity<TechnicianSkillDto> updateSkill(
                ...
                Long technicianId = currentUserProvider.getCurrentUserId();
                dto.setIdUser(technicianId);
                TechnicianSkill updated = technicianSkillService.updateSkill(idSkill,
                                technicianSkillMapper.toModel(dto));
